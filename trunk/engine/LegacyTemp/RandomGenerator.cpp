...

{
  ...
  if (query)

    {
      ...
      std::cout<< "Enter random number seed, 0<seed<65000." << std::endl;
...

{
  ...
  for (j=1;j<=43;j++)        		// make initial numbers.

    {
      ...
      if (word1[j] > CONST1) word1[j] = CONST1;
...

{
  ...
  return ((((double)Next())/(CONST1 + 4))*range);
...

{
  ...
  double theta = 2*M_PI*SampleUniform();
...
// are a std::vector of alphas such that
...
tRandomGenerator::SampleDirichlet(const std::vector<double>& mean,

				  double precision,

				  std::vector<double>& rgprob)
